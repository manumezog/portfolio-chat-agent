...
- **RAG chat** — answers grounded in curated knowledge documents, not LLM memory
- **Cloned voice** — every response is read aloud in Manuel's own cloned voice, generated by [F5-TTS](https://github.com/SWivid/F5-TTS) on a Modal serverless GPU. Click 🔊 to mute.
- **Secure voice proxy** — the Modal API key never reaches the browser; the HF Spaces backend acts as a proxy
- **Multi-turn memory** — session-scoped conversation history for follow-up questions
...
FastAPI backend (api.py)
  ...
  "Find 8 most relevant knowledge chunks"
        │
        ▼
  Answer chain (Gemini 2.5 Flash, max 800 tokens)
  "Answer using only retrieved context"
        │
        ▼
  { answer, session_id }
        │
        │  POST /tts  { text }   ← browser calls this after rendering answer
        ▼
FastAPI /tts proxy
  ├── Rate limiter (10 req/min/IP)
  ├── Strip markdown, cap at 1000 chars
  ├── Load voice sample from F5_REF_AUDIO_B64 secret
        │
        ▼
  Modal GPU endpoint (F5-TTS, T4)
  "Clone voice from reference audio + synthesise answer text"
        │
        ▼
  audio/wav → browser plays automatically
```
...
| TTS | F5-TTS (zero-shot voice cloning) on Modal serverless GPU (T4) |
| API | FastAPI + uvicorn |
...
2. Add Space secrets (Settings → Variables and secrets):
   ...
   | `F5_SERVER_URL` | Modal F5-TTS endpoint URL |
   | `F5_API_KEY` | Modal API key (from your Modal `tts-secrets`) |
   | `F5_REF_TEXT` | Transcript of your voice sample clip |
   | `F5_REF_AUDIO_B64` | Base64-encoded voice sample audio (see Voice cloning below) |
...
> **Note:** Free tier Spaces sleep after 48h of inactivity. The first request after sleep takes ~30s to cold-start. The Modal GPU also cold-starts on the first TTS call (~30-60s); subsequent calls within the 10-minute keep-alive window are fast.

---

## Voice cloning

Responses are read aloud using [F5-TTS](https://github.com/SWivid/F5-TTS) zero-shot voice cloning running on a [Modal](https://modal.com) serverless T4 GPU.

**How it works:**
1. A short reference audio clip of Manuel's voice is stored as a base64 secret (`F5_REF_AUDIO_B64`) on HF Spaces — no binary file in git
2. On startup, the backend decodes it into memory
3. On each `/tts` request, the backend strips markdown from the answer text and forwards it to the Modal endpoint along with the reference audio
4. Modal synthesises speech in the cloned voice and returns WAV audio
5. The browser plays it automatically; the user can mute with the 🔊 button

**To set up your own cloned voice:**
```bash
# Generate base64 of your voice sample (any short clean recording, MP3/MP4/WAV)
python -c "import base64; print(base64.b64encode(open('your_sample.mp4','rb').read()).decode())" > sample_b64.txt
# Add the contents of sample_b64.txt as HF Secret F5_REF_AUDIO_B64
```

The Modal endpoint (`modal_tts_server.py` in the podcaster repo) must be deployed separately:
```bash
...
| Token exhaustion (long outputs) | LLM `max_output_tokens=800`; model instructed to summarise if long |
...
| API key exposure | Keys loaded from `.env` / HF Space secrets, never committed |
| TTS GPU abuse | TTS rate-limited (10 req/min/IP); input capped at 1000 chars |
| Voice sample exposure | Reference audio stored as HF Secret (base64), never in git |
